...
template <class TempGPUBuffer, typename... BufferAllocationArgs> class KernelBase {
  ...
  protected:
    ...
    void run(const Settings &settings,
             const in_t *inputPtr,
             out_t *outputPtr,
             const float *inputLabelsPtr,
             float *outputLabelsPtr,
             float *outputMappingPtr,
             int64_t batchSize,
             int64_t groups,
             int64_t inputHeight,
             int64_t inputWidth,
        ...
        for (size_t i = 0; i < paramsCpu.size(); i += groups) {
            ...
            setGeometricTransform(
                img, panAngle(rnd), tiltAngle(rnd), rotationAngle(rnd), arScaleX * scaleX, arScaleY * scaleY);
